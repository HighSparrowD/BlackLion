...
class TeleBot:
    ...
    def remove_next_step_handler(self, user, handler):
        self.next_step_backend.remove_handler(user, handler)
    ...
    def register_next_step_handler(self, message: types.Message, callback: Callable, chat_id:[Optional[int]]=None, *args, **kwargs) -> None:
        """
    ...
    def register_next_step_handler_by_chat_id(
        ...
        self.next_step_backend.register_handler(chat_id, handler)
        return handler
    ...
    @staticmethod
    def _build_handler_dict(handler, pass_bot=False, user_id=None, is_multihandler=False, **filters):
        """
        ...
        return {
            'function': handler,
            'pass_bot': pass_bot,
            'is_multihandler': is_multihandler,
            'user_id': user_id,
    ...
    def message_handler(
            ...
            is_multihandler: Optional[bool]=False,
            **kwargs):
        """
    ...
    def register_message_handler(self, callback: Callable, content_types: Optional[List[str]]=None, commands: Optional[List[str]]=None,
            regexp: Optional[str]=None, func: Optional[Callable]=None, chat_types: Optional[List[str]]=None, pass_bot: Optional[bool]=False, user_id=Optional[int], is_multihandler: [Optional[bool]]=False, **kwargs):
        """
    ...
    def register_callback_query_handler(self, callback: Callable, func: Callable, pass_bot: Optional[bool]=False, user_id: Optional[int]=None, **kwargs):
        """
        ...
        :return: None
        """
        handler_dict = self._build_handler_dict(callback, user_id=user_id, func=func, pass_bot=pass_bot, **kwargs)
        self.add_callback_query_handler(handler_dict)
        return handler_dict
    ...
    def _test_message_handler(self, message_handler, message):
        ...
        if message_handler['user_id'] and message.from_user.id != message_handler['user_id']:
            return False
            ...
            if handlers:
                ...
                for handler in handlers:
                    if self._test_message_handler(handler, message):
                        if handler["is_multihandler"]:
                            mHandler = handler
                        elif handler.get('pass_bot', False):
                            result = handler['function'](message, bot=self)
                            return
                        else:
                            result = handler['function'](message)
                            return
                        if not handler["is_multihandler"] and not isinstance(result, ContinueHandling):
                            break

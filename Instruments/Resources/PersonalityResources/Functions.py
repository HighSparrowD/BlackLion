...
def set_path_prefixes(input_prefix, output_prefix):
    global input_path_prefix, output_path_prefix
    input_path_prefix = input_prefix
...
def create_generic_resources(location):
    ...
    file = pandas.read_csv(f"{input_path_prefix}{location}.csv", usecols=["Name", "ENG", "RUS", "UKR"])
    ...
    # Save output
    en_dataframe.to_csv(f"{output_path_prefix}Core/Resources/{location}/{location}EN.csv", index=False)
    ru_dataframe.to_csv(f"{output_path_prefix}Core/Resources/{location}/{location}RU.csv", index=False)
    uk_dataframe.to_csv(f"{output_path_prefix}Core/Resources/{location}/{location}UK.csv", index=False)
...
def create_prices_resource():
    ...
    file = pandas.read_csv(f"{input_path_prefix}Prices.csv", usecols=["Name", "Currency", "Price"], )
    ...
    # Save output
    usd_dataframe.to_csv(f"{output_path_prefix}Core/Resources/Prices/PricesUSD.csv", index=False)
    eur_dataframe.to_csv(f"{output_path_prefix}Core/Resources/Prices/PricesEUR.csv", index=False)
    uah_dataframe.to_csv(f"{output_path_prefix}Core/Resources/Prices/PricesUAH.csv", index=False)
    czk_dataframe.to_csv(f"{output_path_prefix}Core/Resources/Prices/PricesCZK.csv", index=False)
    pln_dataframe.to_csv(f"{output_path_prefix}Core/Resources/Prices/PricesPLN.csv", index=False)
    points_dataframe.to_csv(f"{output_path_prefix}Core/Resources/Prices/PricesPoints.csv", index=False)